...
if (!class_exists('WC_Currency_Converter')) {
	...
	class WC_Currency_Converter {
		...
		function currency_conversion_js() {
			...
			wp_enqueue_script( 'wc_currency_converter', get_template_directory_uri() .'/lib/plugins/currency-converter/assets/js/conversion.js', array( 'jquery', 'moneyjs', 'accountingjs', 'jquery-cookie' ), Null, true );
		...
		function update_order_meta( $order_id ) {
			global $woocommerce;
			var_dump($_COOKIE['woocommerce_current_currency']); die();
