...
class YA_Posts_Widget extends YA_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'sw_posts', 'description' => __('SW Posts', 'yatheme'));
		parent::__construct('sw_posts', __('SW Posts', 'yatheme'), $widget_ops);
...
class YA_Slider_Widget extends YA_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'ya_slider', 'description' => __('YA Slider', 'yatheme'));
		parent::__construct('ya_slider', __('YA Slider', 'yatheme'), $widget_ops);
		$this->base = dirname(__FILE__);
	}

	public function add_script_slider() {
		$script = '';
		$script .= '<script type="text/javascript">
			jQuery(document).ready(function($) {
				$(".slideshow").each(function(){
				$("#"+this.id).jCarouselLite({
				auto: false,
				circular: true,
				autoWidth: true,
				responsive: true,
...
class YA_Slider_Content_Widget extends YA_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'ya_slider_content', 'description' => __('YA Slider Content', 'yatheme'));
		parent::__construct('ya_slider_content', __('YA Slider Content', 'yatheme'), $widget_ops);
		$this->base = dirname(__FILE__);
	}
}
...
class YA_Category_List_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_category_list', 'description' =>__('YA Category List header widget', 'yatheme'));
		parent::__construct('ya_category_list', __('YA List Category', 'yatheme'), $widget_ops);
	}
}
...
class YA_Top_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_top', 'description' =>__('YA top header widget', 'yatheme'));
		parent::__construct('ya_top', __('YA Top Widget', 'yatheme'), $widget_ops);
	}
}

class YA_Tabber_Widget extends YA_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'ya_tabber', 'description' =>__('YA Tabber', 'yatheme'));
		parent::__construct('ya_tabber', __('YA Tabber', 'yatheme'), $widget_ops);
	}
}
...
class YA_Top_Rate_Product extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_top_rate_product woocommerce widget_top_rated_products', 'description' =>__('YA Top Rated Product Widget', 'yatheme'));
		parent::__construct('top_rated_product', __('YA Top Rated Product Widget', 'yatheme'), $widget_ops);
	}
}
...
class YA_Best_Seller_Product extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_best_seller_product', 'description' =>__('YA best seller product woocommerce', 'yatheme'));
		parent::__construct('ya_best_seller_product', __('YA best seller products', 'yatheme'), $widget_ops);
	}
}
...
class YA_Related_Product_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_relate_product', 'description' =>__('YA related products woocommerce', 'yatheme'));
		parent::__construct('ya_relate_product', __('YA related products', 'yatheme'), $widget_ops);
	}
...
class YA_Categories_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_categories', 'description' =>__('YA Categories', 'yatheme'));
		parent::__construct('ya_categories', __('YA Catetories', 'yatheme'), $widget_ops);
	}
}
...
class YA_Feature_Product_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_feature_product', 'description' =>__('YA feature product woocommerce', 'yatheme'));
		parent::__construct('ya_feature_product', __('YA Feature Products', 'yatheme'), $widget_ops);
	}
}
...
class YA_Gallery_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_gallery', 'description' =>__('YA Gallery', 'yatheme'));
		parent::__construct('ya_gallery', __('YA Gallery', 'yatheme'), $widget_ops);
	}
	
	public function add_script_gallery() {
		$script = '';
		$script .= '<script type="text/javascript">
			jQuery(document).ready(function($) {
				$(".widget-photobox-gallery").each(function(){
					$("#" + this.id).carousel({
						interval: 4000
					})
					$("#" + this.id).photobox("a");
					// or with a fancier selector and some settings, and a callback:
					$("#" + this.id).photobox("a:first", { thumbs:false, time:0 }, imageLoaded);
...
class YA_Default_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'sw_default', 'description' =>__('WP Default Widget', 'yatheme'));
		parent::__construct('sw_default', __('WP Default Widget', 'yatheme'), $widget_ops);
	}
}
...
class YA_Social_Widget extends WP_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_social', 'description' => __('YA Social Networks', 'yatheme'));
		parent::__construct('ya_social', __('YA Social', 'yatheme'), $widget_ops);
	...
	function widget($args, $instance){
		...
		<ul>
			...
			<li><a href="<?php echo $social['link']; ?>"
				title="<?php if (empty($match)) echo $social['icon']; ?>"> <?php if (empty($match)) echo $social['icon']; else { ?>
					<i class="<?php echo $social['icon']?>"></i> <?php }?>
			</a></li>
			<?php } ?>
		</ul>
		<?php
...
<p>
	...
	<label for="<?php echo $id_j_icon; ?>"><?php _e('icon-* | title', 'yatheme')?>
	</label> <input class="widefat" id="<?php echo $id_j_icon;?>"
		name="<?php echo esc_attr($name_j_icon); ?>" type="text"
		value="<?php echo $social['icon']; ?>"><br>
	<?php }?>
	<label for="<?php echo $id_j_link; ?>"><?php _e('Link ', 'yatheme')?> </label>
	<input class="widefat" id="<?php echo $id_j_link;?>"
		name="<?php echo esc_attr($name_j_link); ?>" type="text"
		value="<?php echo $social['link']; ?>">
</p>
<?php
		}
...
<p>
	<strong>Add a new social network</strong><br> <label
		for="<?php echo $id_i_icon; ?>"><?php _e('Classname for Icon or Title', 'yatheme'); ?>
	</label> <input class="widefat" id="<?php echo $id_i_icon; ?>"
	...
	<label for="<?php echo $id_i_link; ?>"><?php _e('Link ', 'yatheme')?> </label>
	<input class="widefat" id="<?php echo $id_i_link; ?>"
		name="<?php echo $name_i_link; ?>" type="text" value=""
		placeholder="#" />
</p>
<?php
	}
...
//		return $this->menu_assignments[$hash];
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. __( 'Widget Display', 'yatheme' ) .  '</label>';
	...
	private function render_wdisplayxx($widget_display){
		...
		foreach ($display_checkbox['users'] as $value => $name) { ?>
			...
			<input type="checkbox" id="<?php $this->widget->get_field_id($value)?>"
				...
				/>
			<label for="<?php echo $this->widget->get_field_id($value); ?>"><?php $name;?></label><br/>
		<?php } ?>
		</p>
