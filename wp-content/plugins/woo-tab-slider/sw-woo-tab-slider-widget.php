...
function load_woo_tab_slider_script(){
        if (!is_admin() && !defined('SW_WOO_TAB_SLIDER')) {      
            define('SW_WOO_TAB_SLIDER', 'ASSETS SW WOO TAB SLIDER CONTENT');
			wp_register_style( 'woo-tab-styles', plugins_url('css/style.css', __FILE__) );
			wp_enqueue_style('woo-tab-styles');
			wp_register_style( 'woo-tab-slider', plugins_url('css/slider.css', __FILE__) );
			wp_enqueue_style('woo-tab-slider');
			
            wp_register_script( 'wooslider-tab-js', plugins_url( '/js/simpletabs.js', __FILE__ ),array(), null, true );		
            if (!wp_script_is('wooslider-tab-js')) {
				wp_enqueue_script('wooslider-tab-js');
			} 
...
add_action('wp_enqueue_scripts', 'load_woo_tab_slider_script', 10);
/**
...
class sw_woo_tab_slider_widget extends WP_Widget {
	...
	function sw_woo_tab_slider_widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'sw_woo_tab_slider', __('Sw Woo Tab Slider widget', 'yatheme'), $widget_ops, $control_ops );
	}
